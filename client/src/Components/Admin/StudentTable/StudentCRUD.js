...
//   // Calculate the total number of pages based on the number of students and itemsPerPage
//   const totalPages = Math.ceil(AllStud.length / itemsPerPage);

//   // Get the slice of students data based on the current page
...
//             SetAllStud(response.data.data);
//           } else {
//             toast.error('Student Details Search cannot be done. Please check it!!!');
//           }
//         })
//         .catch((error) => {
//           console.error(error);
      ...
      axios
        ...
        .then((response) => {
          if (response.data.status) {
            ...
            SetAllStud([]); // Set an empty array to clear the existing data when search returns no results
          }
        })
        .catch((error) => {
          console.error(error);
  ...
  return (
    <>
      ...
      <Container fluid>
        ...
        {Array.isArray(AllStud) && AllStud.length > 0 ? ( // Check if there are students to display
          <>
            <Row className='m-auto p-auto'>
              <Col>
                <Table responsive striped bordered hover className='table-adjust'>
          ...
          // If there are no students to display, show appropriate message
          <Row className='m-auto p-auto'>
